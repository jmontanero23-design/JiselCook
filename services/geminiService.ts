...
export async function generateRecipeImage(recipeTitle: string, recipeDescription: string): Promise<string | null> {
    // Use the new robust image generation service
    const { generateRecipeImage: genImage } = await import('./imageGenerationService');
    return genImage(recipeTitle, recipeDescription);
}
