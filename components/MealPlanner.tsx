...
export const MealPlanner: React.FC<MealPlannerProps> = ({ pantryItems, userProfile }) => {
  ...
  const handleGenerate = async () => {
    setIsLoading(true);
    try {
      const ingredients = pantryItems.map(name => ({ name, amount: '', unit: '' }));
      const newPlan = await generateMealPlan(userProfile, ingredients);
      if (newPlan) {
              ...
              {day.meals.map((meal, idx) => (
                <div key={idx} className={idx > 0 ? "pt-4 md:pt-0 md:pl-6" : ""}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-xs font-bold uppercase px-2 py-1 rounded-md ${meal.type === 'Breakfast' ? 'bg-amber-100 text-amber-700' :
