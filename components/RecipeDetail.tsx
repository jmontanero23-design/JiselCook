...
import { remixRecipe, generateRecipeImage, findCookingResources, getNutritionalInsights } from '../services/geminiService';
...
export const RecipeDetail: React.FC<RecipeDetailProps> = ({ 
  ...
  const handleFindResources = async () => {
      ...
      try {
          const results = await findCookingResources(recipe.title);
        ...
        <div className="flex items-center space-x-2">
             ...
             <button 
                ...
                className={`flex items-center space-x-2 px-3 py-2 rounded-lg font-medium transition-colors text-sm ${
                    showRemixInput ? 'bg-indigo-100 text-indigo-700' : 'text-indigo-600 hover:bg-indigo-50'
                }`}
             >
                <Wand2 size={18} />
        ...
        <div className="flex flex-wrap gap-4 mb-12">
             <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors text-sm font-medium ${
                    showResources ? 'bg-sky-50 border-sky-200 text-sky-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
             >
                {isLoadingResources ? <Loader2 className="animate-spin" size={16} /> : <BookOpen size={16} />}
             ...
             <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors text-sm font-medium ${
                    showNutrition ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
             >
        ...
        {showNutrition && nutritionInfo && (
            <div className="mb-12 bg-emerald-50 rounded-xl p-6 border border-emerald-100 animate-in slide-in-from-top-2">
                ...
                <div className="prose prose-emerald prose-sm max-w-none">
                    <div className="whitespace-pre-wrap">{nutritionInfo}</div>
                </div>
            </div>
        )}
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    {missingIngredients.length > 0 && (
                        <button
                            ...
                            className={`text-xs font-bold px-3 py-2 rounded-lg flex items-center gap-2 transition-colors ${
                                allMissingAdded 
