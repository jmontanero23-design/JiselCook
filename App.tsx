...
export default function App() {
  ...
  const handleSurpriseMe = async () => {
    ...
    setView('home');
    try {
      const surpriseRecipe = await generateSurpriseRecipe(userProfile);
      if (surpriseRecipe) {
        setRecipes([surpriseRecipe]);
        setView('recipes');

        try {
          const imageUrl = await generateRecipeImage(surpriseRecipe.title, surpriseRecipe.description);
          if (imageUrl) {
            setRecipes(prev => prev.map(r => r.id === surpriseRecipe.id ? { ...r, imageUrl } : r));
          }
        } catch (e) {
          console.error(`Failed to generate image for ${surpriseRecipe.title}`, e);
        }
      } else {
        alert("Chef couldn't come up with a surprise right now. Try again!");
      }
  ...
  const handleImageCaptured = async (file: File) => {
    ...
    try {
      ...
      const ingredients = await import('./services/geminiService').then(m => m.identifyIngredientsFromImage(file));

      if (ingredients.length === 0) {
        alert("Could not identify any ingredients. Please try again.");
        return;
      }

      // Update pantry with found ingredients
      handleIngredientsDetected(ingredients.map(i => i.name));
